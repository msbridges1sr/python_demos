...
def find_longest_seq(case_list: List[int]) -> Tuple[List[int], int]:
    ...
    for index, item in enumerate(case_list):
        ...
        # If next item not in-sequence, start a new seq_indices_list if necessary
        else:
            if len(seq_indices_list) > 1:
                results_list.append(seq_indices_list)
                seq_indices_list = []
    ...
    for index, sublist in enumerate(results_list):
        ...
        answer_seq_values_list = [case_list[i] for i in results_list[index_of_max]]
...
"""
Case #1: [1, 2, 3]
Longest sequence: [2, 3], max length: 2

Case #2: [1, 2, 3, 5, 6, 7, 9, 10, 11, 13, 14, 15]
First of 3 sequences of max length: [5, 6, 7], max length: 3
